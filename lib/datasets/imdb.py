...
class imdb(object):
    ...
    #fast rcnn don't have this property~~~~~~~~
    ...
    def append_flipped_images(self):
          ...
          for i in xrange(num_images):
             ...
             assert (boxes[:, 2] >= boxes[:, 0]).all()  # 2016.4.23 when train fast rcnn, we don't zhishi diao , train faster, if hold this, it will have mistake~~~
             entry = {'boxes' : boxes,
                      'gt_overlaps' : self.roidb[i]['gt_overlaps'],
                      'gt_classes' : self.roidb[i]['gt_classes'],
                      'flipped' : True}
    ...
    def create_roidb_from_box_list(self, box_list, gt_roidb):
        ...
        assert len(box_list) == self.num_images, \
                'yiban shi .mat de wenti Number of boxes must match number of ground-truth images'
        roidb = []
        for i in xrange(self.num_images):
            boxes = box_list[i]
